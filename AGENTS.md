...
- Put helper-only structs/classes/functions in an appropriate utility/helper header/source rather than embedding them directly in primary implementation files. For example, dT-only diagnostics and regulation helpers belong in `src/DEM/utils/DynamicThreadHelpers.hpp` or a similarly named helper file, not directly in `dT.h`/`dT.cpp`.
