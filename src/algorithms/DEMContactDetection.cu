...
void contactDetection(std::shared_ptr<JitHelper::CachedProgram>& bin_sphere_kernels,
        ...
        *scratchPad.getDualStructHost("numActiveBinsForTri") =
        ...
        if (*scratchPad.numPrimitiveContacts > 0) {
            ...
            if (solverFlags.useSimplePatchCombination) {
                ...
                // Safe RefBranch-style path: preserve the existing contact-type segments and sort/group patch pairs
                // inside each segment. This keeps primitive order, geomToPatchMap, and patch-contact slots aligned with
                // dT's patch-force and history-migration assumptions.
                if (!primitiveContactArraysAreSortedByType) {
                    if (blocks_needed_for_patch_ids > 0) {
                    ...
                    cubDEMSortByKeys<contact_t, contactPairs_t>(granData->contactTypePrimitive, contactType_sorted,
                    ...
                    if (blocks_needed_for_patch_ids > 0) {
                        gatherByIndex<bodyID_t>
                            <<<dim3(blocks_needed_for_patch_ids), dim3(DEME_MAX_THREADS_PER_BLOCK), 0, this_stream>>>(
                                granData->idPrimitiveA, idA_sorted, type_indices_sorted, numTotalCnts);
                        DEME_GPU_DEBUG_SYNC(this_stream);
                        gatherByIndex<bodyID_t>
                            <<<dim3(blocks_needed_for_patch_ids), dim3(DEME_MAX_THREADS_PER_BLOCK), 0, this_stream>>>(
                                granData->idPrimitiveB, idB_sorted, type_indices_sorted, numTotalCnts);
                        DEME_GPU_DEBUG_SYNC(this_stream);
                        gatherByIndex<notStupidBool_t>
                            <<<dim3(blocks_needed_for_patch_ids), dim3(DEME_MAX_THREADS_PER_BLOCK), 0, this_stream>>>(
                                granData->contactPersistency, contactPersistency_sorted, type_indices_sorted,
                                numTotalCnts);
                        DEME_GPU_DEBUG_SYNC(this_stream);
                        gatherByIndex<patchIDPair_t>
                            <<<dim3(blocks_needed_for_patch_ids), dim3(DEME_MAX_THREADS_PER_BLOCK), 0, this_stream>>>(
                                contactPatchPairs, patchPairs_sorted, type_indices_sorted, numTotalCnts);
                        DEME_GPU_DEBUG_SYNC(this_stream);
                        ...
                        DEME_GPU_CALL_ASYNC(cudaMemcpyAsync(granData->contactTypePrimitive, contactType_sorted,
                                            ...
                                            this_stream);
                        DEME_GPU_CALL_ASYNC(
                            cudaMemcpyAsync(granData->contactPersistency, contactPersistency_sorted,
                ...
                contact_t* unique_types = (contact_t*)scratchPad.allocateTempVector(
                    "simple_unique_types", NUM_SUPPORTED_CONTACT_TYPES * sizeof(contact_t));
                contactPairs_t* type_counts = (contactPairs_t*)scratchPad.allocateTempVector(
                    "simple_type_counts", NUM_SUPPORTED_CONTACT_TYPES * sizeof(contactPairs_t));
                scratchPad.allocateDualStruct("simple_numUniqueTypes");
                cubDEMRunLengthEncode<contact_t, contactPairs_t>(
                    granData->contactTypePrimitive, unique_types, type_counts,
                    scratchPad.getDualStructDevice("simple_numUniqueTypes"), numTotalCnts, this_stream, scratchPad);
                scratchPad.syncDualStructDeviceToHost("simple_numUniqueTypes");
                size_t numTypes = *scratchPad.getDualStructHost("simple_numUniqueTypes");

                contact_t* host_unique_types = nullptr;
                contactPairs_t* host_type_counts = nullptr;
                if (numTypes > 0) {
                    host_unique_types = new contact_t[numTypes];
                    host_type_counts = new contactPairs_t[numTypes];
                ...
                patchIDPair_t* unique_patch_pairs =
                    (patchIDPair_t*)scratchPad.allocateTempVector("simple_unique_patch_pairs", patch_arr_bytes);
                contactPairs_t* patch_pair_counts = (contactPairs_t*)scratchPad.allocateTempVector(
                    "simple_patch_pair_counts", numTotalCnts * sizeof(contactPairs_t));
                scratchPad.allocateDualStruct("simple_numUniquePatchPairs");

                typeStartCountPatchMap_thisStep.SetAll({0, 0});
                size_t totalUniquePatchPairs = 0;
                size_t prim_offset = 0;
                for (size_t i = 0; i < numTypes; i++) {
                    const contact_t thisType = host_unique_types[i];
                    const contactPairs_t count = host_type_counts[i];
                    if (count == 0) {
                        continue;
                    }
                    ...
                    cubDEMRunLengthEncode<patchIDPair_t, contactPairs_t>(
                        ...
                        scratchPad.getDualStructDevice("simple_numUniquePatchPairs"), count, this_stream, scratchPad);
                    scratchPad.syncDualStructDeviceToHost("simple_numUniquePatchPairs");
                    const size_t numUniqueInSegment = *scratchPad.getDualStructHost("simple_numUniquePatchPairs");

                    const size_t newTotalSize = totalUniquePatchPairs + numUniqueInSegment;
                    ...
                    contactPairs_t* isNewSegmentGroup = (contactPairs_t*)scratchPad.allocateTempVector(
                        "simple_isNewSegmentGroup", count * sizeof(contactPairs_t));
                    const size_t blocks_needed_for_mark =
                    ...
                    if (blocks_needed_for_mark > 0) {
                        markNewPatchPairGroups<<<dim3(blocks_needed_for_mark), dim3(DEME_MAX_THREADS_PER_BLOCK), 0,
                                                 this_stream>>>(patchPairs_sorted + prim_offset, isNewSegmentGroup,
                                                                count);
                        DEME_GPU_DEBUG_SYNC(this_stream);
                    }
                    cubDEMInclusiveScan<contactPairs_t, contactPairs_t>(
                        isNewSegmentGroup, granData->geomToPatchMap + prim_offset, count, this_stream, scratchPad);
                    if (count > 0 && totalUniquePatchPairs > 0) {
                        addOffsetToArray<<<dim3(blocks_needed_for_mark), dim3(DEME_MAX_THREADS_PER_BLOCK), 0,
                                           this_stream>>>(granData->geomToPatchMap + prim_offset,
                                                          static_cast<contactPairs_t>(totalUniquePatchPairs), count);
                        DEME_GPU_DEBUG_SYNC(this_stream);
                    }

                    if (numUniqueInSegment > 0) {
                        typeStartCountPatchMap_thisStep[thisType] = {static_cast<contactPairs_t>(totalUniquePatchPairs),
                                                                     static_cast<contactPairs_t>(numUniqueInSegment)};
                        const size_t blocks_needed_for_decode =
                        ...
                        setFirstFlagToOne<<<1, 1, 0, this_stream>>>(isNewSegmentGroup, count);
                        DEME_GPU_DEBUG_SYNC(this_stream);
                        cubDEMSelectFlagged<bodyID_t, contactPairs_t>(
                            idA_sorted + prim_offset, contactPatchIsland.data() + totalUniquePatchPairs,
                            isNewSegmentGroup, scratchPad.getDualStructDevice("simple_numUniquePatchPairs"), count,
                            this_stream, scratchPad);
                    }

                    scratchPad.finishUsingTempVector("simple_isNewSegmentGroup");
                ...
                *scratchPad.numContacts = totalUniquePatchPairs;
                ...
                scratchPad.finishUsingTempVector("simple_unique_patch_pairs");
                scratchPad.finishUsingTempVector("simple_patch_pair_counts");
                scratchPad.finishUsingDualStruct("simple_numUniquePatchPairs");
                if (host_unique_types) {
                    delete[] host_unique_types;
                }
                if (host_type_counts) {
                    delete[] host_type_counts;
                }
                scratchPad.finishUsingTempVector("simple_unique_types");
                scratchPad.finishUsingTempVector("simple_type_counts");
                scratchPad.finishUsingDualStruct("simple_numUniqueTypes");
            } else {
                ...
                // Stable sort by contact type to preserve patch-pair order within each type segment. dT's patch-history
                // migration relies on this invariant: patch contact IDs and geomToPatchMap are dense in type segments
                // and ordered by patch pair inside each segment.
                ...
                {
                    // This route is opt-in. It starts from the same type/patch-pair sorted primitive arrays as the
                    // simple path, then adds an island label as a secondary key so patch contacts remain type-segmented
                    // and patch-pair ordered before history mapping. Select group contact types (one per group).
                    ...
                    if (numActiveTriKeys > 0) {
                        ...
                        if (numUniqueActiveTri > 0) {
                            ...
                            // ---- Option B: Precompute active-triangle neighbor positions (in activeTriKeysUnique)
                            // once. triNeighbor{1,2,3} are precomputed global edge-neighbor triangle IDs, but we still
                            // need a per-step mapping "neighbor triID -> position in the ACTIVE list" to avoid
                            // per-iteration binary searches.
                            ...
                            // We check only the *last* iteration in a small batch (default: 4) to avoid host/device
                            // sync each iteration. Worst-case wasted work is (batchSize-1) extra iterations.
            ...
            // Shared cleanup for arrays allocated before the complex/simple branch.
            if (!solverFlags.useSimplePatchCombination) {
    ...
    if (solverFlags.useStablePatchIslandIDs && !solverFlags.useSimplePatchCombination &&
        *scratchPad.numPrimitiveContacts > 0) {
        ...
        xt.add(granData->previous_idPrimitiveA, granData->idPrimitiveA, primitive_id_arr_bytes);
        xt.add(granData->previous_idPrimitiveB, granData->idPrimitiveB, primitive_id_arr_bytes);
        xt.add(granData->previous_contactTypePrimitive, granData->contactTypePrimitive, primitive_type_arr_bytes);
        xt.run(dev, dev, this_stream);
