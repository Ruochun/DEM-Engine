...
__device__ bool checkSphereOuterTriPrismBarrier(const T1& A,
    ...
    const T2 overlap_area = (T2)deme::PI * ((T2)2 * radius * capped_depth - capped_depth * capped_depth);
    overlapArea = overlap_area > (T2)0 ? overlap_area : (T2)0;
    return true;
}
...
__device__ bool checkTriSphereOverlap(const T1& A,           ///< First vertex of the triangle
    ...
    overlapArea = (T2)deme::PI * ((T2)2 * radius * depth - depth * depth);
    return in_contact;
}
